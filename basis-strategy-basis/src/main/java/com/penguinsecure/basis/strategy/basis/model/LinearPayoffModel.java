...
/** Certified linear perpetual/future payoff in explicit scaled units. */
public final class LinearPayoffModel implements PayoffModel {
    private final int modelId;

    public LinearPayoffModel(final int modelId) {
    ...
    public NumericStatus nativeQuantity(
            ...
            final MutableLongResult scratch,
            final MutableLongResult result) {
        if (!supports(instrument) || canonicalExposure < 0) {
            return result.fail(NumericStatus.MALFORMED).status();
        }
        final int shift = instrument.multiplierScale() + instrument.quantityScale() - exposureScale;
        if (shift < -18 || shift > 18) {
            return result.fail(NumericStatus.SCALE_OUT_OF_RANGE).status();
        }
        long multiplier = 1;
        long divisor = instrument.contractMultiplier();
        if (shift >= 0) multiplier = CheckedDecimalMath.powerOfTen(shift);
        else if (CheckedDecimalMath.multiply(
                        divisor, CheckedDecimalMath.powerOfTen(-shift), scratch)
                != NumericStatus.OK) return result.fail(NumericStatus.OVERFLOW).status();
        else divisor = scratch.value();
        return CheckedDecimalMath.multiplyDivide(
                canonicalExposure, multiplier, divisor, rounding, scratch, result);
    }
    ...
    public NumericStatus riskValue(
            ...
            final MutableLongResult scratch,
            final MutableLongResult result) {
        if (!supports(instrument)) return result.fail(NumericStatus.MALFORMED).status();
        return PayoffMath.linearQuoteAmount(
                price,
