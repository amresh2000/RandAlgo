...
public final class BybitNettyWebSocketConnection
        implements VenueConnectionControl, BybitSessionListener {
    ...
    private final Function<BybitSessionListener, ? extends ChannelHandler> handlerFactory;
    private BybitSessionListener sessionListener = BybitSessionListener.NOOP;
    private boolean listenerBound;
    private volatile Channel channel;

    public BybitNettyWebSocketConnection(
            final URI endpoint,
            ...
            final Function<BybitSessionListener, ? extends ChannelHandler> handlerFactory) {
        if (endpoint == null
                || eventLoopGroup == null
                || sslContext == null
                || handlerFactory == null)
    ...
    public void bindSessionListener(final BybitSessionListener listener) {
                                    ...
                                    protected void initChannel(final SocketChannel socket) {
                                        socket.pipeline()
                                                ...
                                                .addLast(
                                                        "handshake-events",
                                                        new HandshakeEvents(
                                                                BybitNettyWebSocketConnection
                                                                        .this));
                                        BybitWebSocketPipeline.install(
                                                socket.pipeline(),
                                                maximumFrameBytes,
                                                handlerFactory.apply(
                                                        BybitNettyWebSocketConnection.this));
                                    }
                                });
    ...
    static final class HandshakeEvents extends ChannelInboundHandlerAdapter {
        private final BybitSessionListener listener;

        HandshakeEvents(final BybitSessionListener listener) {
